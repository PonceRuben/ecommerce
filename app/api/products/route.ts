...
export async function GET(req:NextRequest) {
    try{
        return NextResponse.json<Response>(
            {message: "Productos encontrados", data: productos},
            {status: 200}
        );
    }   catch(error) {
...
export async function POST(req:NextRequest) {
    try {
        ...
        if (!body.name || !body.price || typeof body.name !== "string" || typeof body.price !== "number") {
            return NextResponse.json (
...
export async function PUT(req:NextRequest) {
    ...
    if (!body.name || !body.price || typeof body.name !== "string" || typeof body.price !== "number") {
    return NextResponse.json(
        { message: "Datos inválidos. Se requiere 'name' (string) y 'price' (number)." },
        { status: 400 }
    );
    }

    // Buscar el producto por id
    const productIndex = productos.findIndex((producto) => producto.id === parseInt(id));
    ...
    return NextResponse.json(
    { message: "Producto actualizado con éxito", data: productos[productIndex] },
    { status: 200 }
    );
} catch (error) {
    // Manejo de errores
